    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        ...
        <p className="text-sm text-muted-foreground mt-2 font-mono">
          observability-mcp v{stats?.version ?? "…"} · {stats?.tool_count ?? 0} tools
        </p>
        <p className="text-xs text-muted-foreground mt-2">MCP HTTP port: {stats?.mcp_port ?? 12007}</p>
      </Card>
      <Card>
        ...
        <ul className="text-xs font-mono mt-2 space-y-1 text-muted-foreground">
          ...
          <li>Prometheus: {stats?.prometheus_url ?? "http://127.0.0.1:12001"}</li>
          <li>Loki: {stats?.loki_url ?? "http://127.0.0.1:12002"}</li>
        </ul>
        <p className="text-xs text-muted-foreground mt-3">
          Start stack: <code className="text-primary">mcp-central-docs/monitoring/start-unified-monitoring.ps1</code>
        </p>
      </Card>
      <Card>
        ...
        <p className="text-xs font-mono mt-2 break-all">
          {llm?.configured_sampling_url || "OBSERVABILITY_SAMPLING_BASE_URL / LLM_BASE_URL not set"}
        </p>
      ...
      <Card>
        ...
        <p className="text-sm text-muted-foreground mt-2">
          ...
          <code className="text-primary">PROMETHEUS_PORT</code> (default 12009). That is not the Prometheus server on
          12001.
        </p>
      </Card>
    </div>
