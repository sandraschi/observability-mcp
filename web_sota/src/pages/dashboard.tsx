...
import { Cpu, MemoryStick, HardDrive, Network, RefreshCw, TrendingUp, TrendingDown, Minus } from "lucide-react";
...
interface Metrics {
  ...
  network_io: { bytes_sent: number; bytes_recv: number; packets_sent: number; packets_recv: number };
}

interface HistoryPoint { cpu_percent: number; memory_mb: number; }
...
function Trend({ history, field }: { history: HistoryPoint[]; field: keyof HistoryPoint }) {
  ...
  if (last < avg * 0.95) return <TrendingDown className="w-3 h-3 text-emerald-400" />;
  return <Minus className="w-3 h-3 text-zinc-500" />;
}

function GaugeBar({ value, max = 100, warn = 70, crit = 90 }: { value: number; max?: number; warn?: number; crit?: number }) {
...
export function Dashboard() {
  ...
  async function fetchMetrics() {
    try {
      const result = await callTool("collect_performance_metrics", { service_name: "system" }) as {
        metrics: Metrics; trends: Record<string, string>;
      };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-bold font-mono text-zinc-100">System Metrics</h1>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
              ...
              <span className="text-xs font-mono uppercase tracking-widest">CPU</span>
            </div>
          ...
          <div className="flex items-end justify-between">
            ...
            <Sparkline data={history.map((h) => h.cpu_percent)} color="#f59e0b" />
          </div>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
              ...
              <span className="text-xs font-mono uppercase tracking-widest">Memory</span>
            </div>
          ...
          <GaugeBar value={metrics?.memory_mb ?? 0} max={65536} warn={49152} crit={58982} />
          <div className="text-xs font-mono text-zinc-500">
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
              ...
              <span className="text-xs font-mono uppercase tracking-widest">Disk</span>
            </div>
          </div>
          <div className="text-4xl font-mono font-bold text-zinc-100">
            {loading ? "—" : `${metrics?.disk_usage_percent.toFixed(1)}%`}
          </div>
          <GaugeBar value={metrics?.disk_usage_percent ?? 0} warn={75} crit={90} />
        </div>
        ...
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-zinc-400">
              ...
              <span className="text-xs font-mono uppercase tracking-widest">Network</span>
            </div>
          </div>
          <div className="space-y-2 font-mono text-sm">
            <div className="flex justify-between text-zinc-300">
              <span className="text-zinc-500">↑ sent</span>
              <span>{loading ? "—" : fmtBytes(metrics?.network_io.bytes_sent ?? 0)}</span>
            </div>
            <div className="flex justify-between text-zinc-300">
              <span className="text-zinc-500">↓ recv</span>
              <span>{loading ? "—" : fmtBytes(metrics?.network_io.bytes_recv ?? 0)}</span>
            </div>
            <div className="flex justify-between text-zinc-400 text-xs pt-1 border-t border-zinc-800">
              ...
              <span>{loading ? "—" : (metrics?.network_io.packets_sent ?? 0).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-zinc-400 text-xs">
              <span className="text-zinc-600">pkts recv</span>
              <span>{loading ? "—" : (metrics?.network_io.packets_recv ?? 0).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border border-zinc-800 rounded-lg px-4 py-3 flex items-center gap-3 text-xs font-mono text-zinc-500">
        ...
        Prometheus scrape endpoint active on <span className="text-amber-400">:9090</span> — connect Grafana for historical charting
      </div>
    </div>
  );
}
