...
import { Bell, Send, Loader2, AlertTriangle, Info, AlertCircle, Zap } from "lucide-react";
...
const DEFAULT_ALERTS: AlertConfig[] = [
  { metric_name: "cpu_percent", threshold: 90.0, operator: "gt", severity: "warning", enabled: true },
  { metric_name: "memory_mb", threshold: 1000.0, operator: "gt", severity: "error", enabled: true },
  { metric_name: "error_rate", threshold: 0.05, operator: "gt", severity: "error", enabled: true },
];
...
export function Alerts() {
  ...
  async function sendLog() {
    ...
    try {
      ...
      setSendResult(`sent at ${new Date(result.timestamp).toLocaleTimeString()} → ${result.loki_endpoint}`);
      setLogMsg("");
    } catch (e) {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-zinc-500 font-mono text-sm mt-1">configured thresholds &amp; manual log injection</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xs font-mono uppercase tracking-widest text-zinc-500">Active Alert Configs</h2>
        </div>
        <p className="text-xs font-mono text-zinc-600">
          Loaded at server startup. Edit thresholds in server.py → <span className="text-zinc-400">default_alerts</span> and restart.
        </p>
          ...
          {DEFAULT_ALERTS.map((alert, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-mono text-sm font-semibold">{alert.metric_name}</div>
                  <div className="font-mono text-xs mt-0.5 opacity-70">
                    {alert.operator === "gt" ? ">" : alert.operator === "lt" ? "<" : alert.operator} {alert.threshold.toLocaleString()}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono opacity-70 uppercase">{alert.severity}</span>
                <div className={`w-2 h-2 rounded-full ${alert.enabled ? "bg-current" : "bg-zinc-700"}`} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xs font-mono uppercase tracking-widest text-zinc-500">Manual Log Injection</h2>
        <p className="text-xs font-mono text-zinc-600">
          Send a log entry directly to Loki — useful for annotating deployments or incidents.
        </p>
      ...
      <div className="border border-zinc-800 rounded-lg px-4 py-3 text-xs font-mono text-zinc-600 space-y-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-zinc-400">{import.meta.env.VITE_LOKI_URL ?? "http://localhost:3100"}</span>
        </div>
        <div className="text-zinc-700">
          Set <span className="text-zinc-500">LOKI_URL</span> env var on the backend to point to your Loki instance.
          Use the Log Explorer page to query collected logs.
        </div>
      </div>
    </div>
  );
