"""
FastMCP 3.3+ Dual Transport Configuration
...
Environment Variables:
    ...
    MCP_PORT: Port for HTTP/SSE. Default: 12007 (unified 12000 band; set MCP_PORT to override)
...
ENV_PORT = "MCP_PORT"  # default: 12007
...
def get_transport_config() -> dict:
    ...
    return {
        ...
        "port": int(os.getenv(ENV_PORT, "12007")),
...
Examples:
  ...
  # HTTP mode (web apps)
  python -m {server_name.replace("-", "_")} --http --port 12007

  # Via environment
  MCP_TRANSPORT=http MCP_PORT=12007 python -m {server_name.replace("-", "_")}
""",
    )

    transport_group = parser.add_mutually_exclusive_group()
    transport_group.add_argument("--stdio", action="store_true", help="Run in STDIO (JSON-RPC) mode (default)")
    transport_group.add_argument("--http", action="store_true", help="Run in HTTP Streamable mode (FastMCP 3.3+)")
    ...
    parser.add_argument("--port", type=int, default=None, help=f"Port to listen on (default: ${ENV_PORT} or 12007)")
...
async def run_server_async(mcp_app, args: argparse.Namespace | None = None, server_name: str = "mcp-server") -> None:
    ...
    try:
        if transport == "stdio":
            ...
            from observability_mcp.server import app as composite_app

            host = config["host"]
            port = config["port"]
            logger.info(f"Running composite ASGI on http://{host}:{port} (MCP {config['path']}, REST /api, docs /docs)")
            uv_config = uvicorn.Config(composite_app, host=host, port=port, log_level="info")
